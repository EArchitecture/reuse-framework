...
public class Specs {
  ...
  public class ByExampleSpecification<T, B> implements Specification<T> {
    ...
    private void stringProcessor(Root<T> root, CriteriaBuilder cb, PropertyDescriptor pd, String value, List<Predicate> predicates) {
      EntityType<T> entity = root.getModel();
      Expression<String> exp = (Expression<String>) root.get(entity.getSingularAttribute(pd.getName()));
    ...
    @SuppressWarnings("unchecked")
    private <C extends Comparable<? super C>> boolean comparableProcessor(Root<T> root, CriteriaBuilder cb, List<Predicate> predicates, EntityType<T> entity, PropertyDescriptor pd, Object val,
        Class<C> clazz) {
      boolean ret = false;
